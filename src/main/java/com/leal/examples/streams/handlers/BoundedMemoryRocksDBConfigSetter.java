...
// Copied from: https://github.com/confluentinc/ksql/tree/master/ksqldb-rocksdb-config-setter/src/main/java/io/confluent/ksql/rocksdb
 ...
 */
public class BoundedMemoryRocksDBConfigSetter implements RocksDBConfigSetter, Configurable {
    ...
    static void configure(
        ...
        if (configured.getAndSet(true)) {
            throw new IllegalStateException(
                    "BoundedMemoryRocksDBConfigSetter has already been configured. Cannot re-configure.");
        }

        try {
            final BoundedMemoryRocksDBConfig pluginConfig =
                    new BoundedMemoryRocksDBConfig(config);
    ...
    private static void limitTotalMemory(
            final BoundedMemoryRocksDBConfig config,
            final LruCacheFactory cacheFactory,
            final WriteBufferManagerFactory bufferManagerFactory
    ) {
        final long blockCacheSize =
                config.getLong(BoundedMemoryRocksDBConfig.BLOCK_CACHE_SIZE);
        final long totalMemtableMemory =
                config.getLong(BoundedMemoryRocksDBConfig.WRITE_BUFFER_LIMIT) == -1
                        ? blockCacheSize / 2
                        : config.getLong(BoundedMemoryRocksDBConfig.WRITE_BUFFER_LIMIT);
        final boolean useCacheForMemtable =
                config.getBoolean(BoundedMemoryRocksDBConfig.ACCOUNT_WRITE_BUFFER_AGAINST_CACHE);
        final boolean strictCacheLimit =
                config.getBoolean(BoundedMemoryRocksDBConfig.STRICT_CACHE_LIMIT);

        final double indexFilterBlockRatio =
                config.getDouble(BoundedMemoryRocksDBConfig.INDEX_FILTER_BLOCK_RATIO_CONFIG);
    ...
    private static void configureNumThreads(
        ...
        final int numBackgroundThreads =
                config.getInt(BoundedMemoryRocksDBConfig.N_BACKGROUND_THREADS_CONFIG);
    ...
    public void setConfig(
        ...
        if (!configured.get()) {
            throw new IllegalStateException(
                    "Cannot use BoundedMemoryRocksDBConfigSetter before it's been configured.");
        }
